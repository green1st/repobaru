...
async function importKey(pem, type, algorithm, encoder) {
    switch (type) {
        case 'Ed25519':
        case 'RSASSA-PKCS1-v1_5': {
            ...
            return globalThis.crypto.subtle.importKey('pkcs8', binaryKey.buffer, { name: type, hash: { name: algorithm } }, false, ['sign']);
        }
        case 'HMAC': {
            return globalThis.crypto.subtle.importKey('raw', encoder.encode(pem), { name: type, hash: algorithm }, false, ['sign']);
        }
        default: {
            throw (0, websocket_util_js_1.neverGuard)(type, `Unhandled key type: "${type}"`);
        }
    }
}
/**
 ...
 */
async function signMessage(message, secret, method, algorithm, _pemEncodeMethod = method) {
    const encoder = new TextEncoder();
    const signKeyType = getSignKeyType(secret);
    const key = await importKey(secret, signKeyType, algorithm, encoder);
    const buffer = await globalThis.crypto.subtle.sign({ name: signKeyType }, key, encoder.encode(message));
    switch (method) {
        case 'hex': {
        ...
        default: {
            throw (0, websocket_util_js_1.neverGuard)(method, `Unhandled sign method: "${method}"`);
        }
    }
}
